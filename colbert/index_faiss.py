...
from colbert.indexing.manifest import upsert_segment
...
def main():
    ...
    with Run.context():
        ...
        if args.slices == 1:
            faiss_name = f'ivfpq.{args.partitions}.faiss'
            upsert_segment(args.index_path, name='base', relative_path='.', segment_type='base', faiss_name=faiss_name)
        else:
            Run.warn("Skipping manifest FAISS registration because --slices > 1 is not supported by incremental retrieval.")
