...
from colbert.indexing.loaders import load_doclens, load_pid_mappings
...
class FaissIndex():
    def __init__(self, index_path, faiss_index_path, nprobe, part_range=None):
        ...
        all_pidmaps = load_pid_mappings(index_path, flatten=False)
        ...
        if faiss_part_range is not None:
            ...
            all_pidmaps = all_pidmaps[faiss_part_range.start:faiss_part_range.stop]
        ...
        flat_doclens = flatten(all_doclens)
        flat_pidmaps = flatten(all_pidmaps)
        assert len(flat_doclens) == len(flat_pidmaps), (len(flat_doclens), len(flat_pidmaps))

        total_num_embeddings = sum(flat_doclens)
        self.emb2pid = torch.zeros(total_num_embeddings, dtype=torch.int)

        offset_doclens = 0
        for pid, dlength in zip(flat_pidmaps, flat_doclens):
            self.emb2pid[offset_doclens: offset_doclens + dlength] = int(pid)
            offset_doclens += dlength

        print_message("len(self.emb2pid) =", len(self.emb2pid))

        pool_size = max(1, min(4, multiprocessing.cpu_count()))
        self.parallel_pool = Pool(pool_size)
