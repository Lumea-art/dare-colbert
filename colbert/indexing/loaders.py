...
def load_pid_mappings(directory, flatten=True):
    parts, _, _ = get_parts(directory)

    pidmap_filenames = [os.path.join(directory, 'pidmap.{}.json'.format(filename)) for filename in parts]
    all_pidmaps = []

    for part, filename in zip(parts, pidmap_filenames):
        if os.path.exists(filename):
            all_pidmaps.append(ujson.load(open(filename)))
        else:
            doclens = ujson.load(open(os.path.join(directory, 'doclens.{}.json'.format(part))))
            all_pidmaps.append(list(range(sum(len(x) for x in all_pidmaps), sum(len(x) for x in all_pidmaps) + len(doclens))))

    if flatten:
        all_pidmaps = [x for sub_pidmaps in all_pidmaps for x in sub_pidmaps]
